...
RegisterCommand("givecarplate", function(source, args, rawCommand)
	...
	if source ~= 0 then
		if havePermission(xPlayer) then
			if args[1] == nil then
				TriggerClientEvent('matyr_notify:DoHudText', source, { type = 'inform', text = '/givecarplate [carModel] [plate] [playerID]' })
			else
				if args[2] == nil then
					TriggerClientEvent('matyr_notify:DoHudText', source, { type = 'error', text = _U('none_plate') })
				else
					if args[3] ~= nil then
						local playerName = GetPlayerName(args[3])
						TriggerClientEvent('esx_giveownedcar:spawnVehiclePlate', source, args[1], args[2], args[3],
							playerName, 'player')
					else
						local sourceID = source
						local playerName = GetPlayerName(sourceID)
						TriggerClientEvent('esx_giveownedcar:spawnVehiclePlate', source, args[1], args[2], sourceID,
							playerName, 'player')
					end
				end
			end
		...
		if args[1] == nil then
			...
			if args[2] == nil then
				...
				if args[3] ~= nil then
					...
					local playerName = GetPlayerName(sourceID)
					TriggerClientEvent('esx_giveownedcar:spawnVehiclePlate', sourceID, args[1], args[2], args[3],
						playerName, 'console')
				else
					print('ERROR: you need type playerID')
				end
...
RegisterCommand("delcarplate", function(source, args, rawCommand)
	...
	if source ~= 0 then
		if havePermission(xPlayer) then
			...
			TriggerClientEvent('matyr_notify:DoHudText', source, { type = 'inform', text = _U('del_car', args[1]) })
		end
	end
end, false)
	...
	MySQL.Async.execute(
		'INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (@owner, @plate, @vehicle)',
		{
			...
			if Config.ReceiveMsg then
				TriggerClientEvent('matyr_notify:DoHudText', _source, { type = 'success', text = _U('received_car', string.upper(vehicleProps.plate)) })
			end
		end)
end)
...
function havePermission(xPlayer, exclude)                                                                                                 -- you can exclude rank(s) from having permission to specific commands 	[exclude only take tables]
	if exclude and type(exclude) ~= 'table' then
		exclude = nil; print("^3[esx_giveownedcar] ^1ERROR ^0exclude argument is not table..^0")
