...
local function hide(playerPedId, data)
    local isEmpty = lib.callback.await('s1n_carryandhideintrunk:checkEmptyTrunk', 200,
    ...
    AttachEntityToEntity(playerPedId, data.entity, -1, 0.0, -1.8, 0.5, 0.0, 0.0, 0.0, false, false, false, false, 20,
        true)
...
local function hidePlayer(data)
    ...
    TriggerServerEvent("s1n_carryandhideintrunk:hidePlayer",
        GetPlayerServerId(NetworkGetPlayerIndexFromPed(carryingEntity)), NetworkGetNetworkIdFromEntity(data.entity))
end

local function removePlayerFromTrunk(data)
    TriggerServerEvent("s1n_carryandhideintrunk:stopCarrying",
        GetPlayerServerId(NetworkGetPlayerIndexFromPed(carryingEntity)), NetworkGetNetworkIdFromEntity(data.entity))
